...
{
   TSubmited elm; //параметры одного решения
   unsigned int n,tm,mm,tm1,mm1;
   int r,bl=0,msg,err,again_plagiat;
   char query_str[150],filename[MAX_PATH],filename_snd[MAX_PATH],s[20],ts[50],s_log[300],s_prb[20];
   ...
   while(true) {
      ...
      if (bl) {
         DB.FinishTesting(); //переход сервера в состояние ожидания
      }
      ...
      bl=DB.SelectDataSubmit(&elm,snapshot); //поиск непроверенных решений
      if (bl==0) { //не найдено
         DB.rollback_trans(snapshot);
         msg=DB.wait_events(); //ожидание событий от БД

         if ( !msg || (msg & EVN_STOP_SRV)>0 ) {
			   info2log("Завершение работы");
			   break; //завершение работы сервера
		   }
         continue;
      }

      sprintf(s_log,"Начало проверки решения %d",elm.id_stat);
      info2log(s_log);

      DB.StartTesting(); //переход сервера в состояние проверки решения
	  ...
	  _itoa(elm.id_prb,s_prb,10);
	  StrReplace(compil_prm,"$(id_prb)",s_prb);
      ...
      otchet << "<pre><strong>Сообщения компилятора (и скрипта компиляции):</strong>\n";
      
	  otchet.close();
      strcpy(query_str,compil_prm);

      //sprintf(pt.dir_temp,"%s%d\\",master_cfg->GlobalPaths->DirTemp,DB.get_id());
	  sprintf(pt.dir_temp,"%s%d\\",master_cfg->GlobalPaths->DirTemp,elm.id_stat);
      ...
      if (!FileExists(exe_file)) { //нет выходного файла -> ошибка компиляции
         ...
         //определение полных имен файлов и каталогов необходимых при проверке решения
         sprintf(pt.DirTests,"%s%d\\%s",master_cfg->GlobalPaths->DirProblems,elm.id_prb,
            master_cfg->ProblemPaths->Tests);
         sprintf(pt.DirPreliminary,"%s%d\\%s",master_cfg->GlobalPaths->DirProblems,elm.id_prb,
            master_cfg->ProblemPaths->Preliminary);
		 ...
		 //путь к программе жюри для интерактивных задач
         sprintf(pt.FileInteract,"%s%d\\interact.exe",master_cfg->GlobalPaths->DirProblems,elm.id_prb);
		 if (!FileExists(pt.FileInteract)) {
             //sprintf(pt.FileInteract,"%s%d\\interact.jar",master_cfg->GlobalPaths->DirProblems,elm.id_prb);
             //if (FileExists(pt.FileInteract)) {
				 //исправить!!!! //debug
             //  sprintf(pt.FileInteract,"C:\\Programs\\java\\bin\\java.exe -jar %s%d\\interact.jar",master_cfg->GlobalPaths->DirProblems,elm.id_prb);
			 //}
			 //else
			   pt.FileInteract[0]=0;
		 }
         ...
         sprintf(pt.ListPoints,"%s%d\\%s",master_cfg->GlobalPaths->DirProblems,elm.id_prb,
             master_cfg->ProblemPaths->ListPoints);
         sprintf(pt.ListPreliminary,"%s%d\\%s",master_cfg->GlobalPaths->DirProblems,elm.id_prb,
             master_cfg->ProblemPaths->ListPreliminary);
		 ...
		 */
         ...
         // возможно, вначале нужно сделать предварительную проверку на тестах из условия
         bool prelim = false;
		 if (FileExists(pt.ListPreliminary)) {
            prelim = true;
         }         

         r = RS_ACCEPTED;
		 TSubmited elmSaved = elm;
         if (prelim) {           
           r = TestSolve(&pt,&elm.time,&elm.mem,&n,elm.id_cmp, elm.points, elm.hardlevel, true); //проверить решение на тестах из условия
           n = -n;
         }
         if (r == RS_ACCEPTED) {
           prelim = false;
		   elm = elmSaved;
           r=TestSolve(&pt,&elm.time,&elm.mem,&n,elm.id_cmp, elm.points, elm.hardlevel, false); //проверить решение
         }

         //вычтем назад поправки, но так чтобы не получилось отрицательного времени или памяти
...
*/
         ...
         */
         ...
         if (n==0) {
            sprintf(query_str,"update status set points=%d, id_rsl=%d, "
               "time_work=%.9f,mem_use=%d,uniq_proc=%f %s %s where id_stat=%d",
			      elm.points, r,elm.time,elm.mem,elm.cur_uniq_proc,cmp_id_stat_null,
            ...
            sprintf(query_str,"update status set points=%d, id_rsl=%d, test_no=%d, "
               "time_work=%.9f,mem_use=%d %s where id_stat=%d",
               elm.points, r,n,elm.time,elm.mem,obj_data_null,elm.id_stat);
         }
      ...
      sprintf(s_log,"Завершена проверка решения %d",elm.id_stat);
      info2log(s_log);
      ...
      do {
         ...
         if (err==-1) 
            info2log("Блокировка из-за пересчета статистики другим сервером. "
                     "Попытка сохранить результат еще раз..."); 
      } while(err==-1);

      DB.commit_trans(snapshot);

      sprintf(s_log,"Сохранен результат проверки решения %d",elm.id_stat);
      info2log(s_log);
...
void err(const char *s)
{
   ...
   logfl << ts << " [СЕРВЕР "<<DB.get_id() <<"] ERROR: " << s << endl;
...
void info2log(const char *s)
{
   ...
   GetSystemTime(&sys_tm);
   sprintf(ts,"%02d:%02d:%02d.%03d",sys_tm.wHour,sys_tm.wMinute,sys_tm.wSecond,sys_tm.wMilliseconds);
   logfl << ts << " [СЕРВЕР "<<DB.get_id() <<"] " << s << endl;
}
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR CmdLine, int nCmdShow)
{
   ...
   //выставляем недостающие переменные окружения
   char sys_dir[256];
   ...
   putenv(env_dir);
   //TNS_ADMIN
   sprintf(env_dir,"TNS_ADMIN=C:\\acm\\compilers\\sql");
   putenv(env_dir);
   //NLS_LANG for Oracle
   sprintf(env_dir,"NLS_LANG=RUSSIAN_RUSSIA.CL8MSWIN1251");
   putenv(env_dir);
