...
#      $stat_err=5 if ($i);
   ...
   if (!$stat_err && !($$ptr_id_prb)) {
      ...
      $query="insert into problems(time_lim,mem_lim,id_creator,min_uniq_proc,id_serv) "
            ."values($time_limit,$mem_limit,$id_user,$min_uniq_proc,(select id_serv from compil where id_cmp=$id_compil))";
      $sth=$db->prepare($query);
      $sth->execute();
