...
*/
   ...
   *mem_lim=(unsigned int)cMem;
   ...
   if (ret==RS_ACCEPTED && pt->WAProg!=NULL && run_checker) {
      ...
      //sprintf(WACmd,"%s %s %s",pt->inp,pt->tst,pt->out);
...
unsigned int TestSolve(TPaths *pt, double *time, unsigned int *mem, unsigned int *test_num, int id_cmp)
{
   int i,flpos;
   ...
   *test_num=0;
   ...
   while (!list.eof()) {  //пока есть тесты
      ...
      for (int ck=0;ck<5;ck++) {
         time1=*time;
         mem1=*mem;
         r1=RunProblem(pt,&time1,&mem1,&er,!ck); //прохождение одного теста
         if (!ck) r=r1;
         Sleep(20);
         if (time1>0.1) break;
      ...
      otchet << "<td>" << r << "</td><td>" << setprecision(9) << fixed << time1
