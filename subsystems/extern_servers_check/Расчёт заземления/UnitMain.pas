...
type
  ...
  private
    ...
    procedure MsgBox(Text: PAnsiChar);
...
procedure TForm1.MsgBox(Text: PAnsiChar);
// Намекаем пользователю, что введённые данные некорректны
begin
  Application.MessageBox(Text,'Подсказка',mb_IconAsterisk+mb_Ok);
end;

function TForm1.CheckFields: boolean;
...
begin
  ...
  try
    ...
    if Rz<=0 then raise Exception.Create('');
    inc(i);
    L:=StrToFloat(Edit2.Text);
    if L<=0 then raise Exception.Create('');
    inc(i);
    D:=StrToFloat(Edit3.Text)/1000;
    if D<=0 then raise Exception.Create('');
    inc(i);
    H:=StrToFloat(Edit4.Text);
    if H<=0 then raise Exception.Create('');
    inc(i);
    B:=StrToFloat(Edit5.Text)/1000;
    if B<=0 then raise Exception.Create('');
    inc(i);
    Re:=StrToFloat(Edit6.Text);
    if Re<0 then raise Exception.Create('');
    inc(i);
    Pizm:=StrToFloat(Edit7.Text);
    if Pizm<=0 then raise Exception.Create('');
  except
    case i of
      0: MsgBox('Норма сопротивления заземления введена некорректно');
      1: MsgBox('Длина заземлителя введена некорректно');
      2: MsgBox('Диаметр заземлителя введён некорректно');
      3: MsgBox('Глубина заложения полосы введена некорректно');
      4: MsgBox('Ширина соединительной полосы введена некорректно');
      5: MsgBox('Сопротивление естественных заземлителей введено некорректно');
      6: MsgBox('Удельное сопротивление грунта введено некорректно');
    end;
    Exit;
  end;
  if ComboBox1.ItemIndex<0 then MsgBox('Не выбран тип грунта') else
    if ComboBox2.ItemIndex<0 then MsgBox('Не выбрана влажность грунта') else
      if ComboBox4.ItemIndex<0 then MsgBox('Не выбрано отношение расстояния между заземлителями к их длине') else
        if (RadioButton1.Checked=false) and (RadioButton2.Checked=false) then MsgBox('Не выбрано размещение заземлителей')
          else Result:=true;
end;

procedure TForm1.Button1Click(Sender: TObject);
begin
  // Проверяем исходные данные и, если они корректные, начинаем расчёты
  if CheckFields then
  begin
...
//    MsgBox('Некорректное значение удельного сопротивления!');
    Exit;
  end;
  ...
  begin
    ...
    begin
      MsgBox(PAnsiChar('Для выбранного вида грунта минимальное значение удельного сопротивления равно '+FloatToStr(minPizm)));
      ComboBox3.ItemIndex:=-1;
    end;
    if Pizm>maxPizm then
    begin
      MsgBox(PAnsiChar('Для выбранного вида грунта максимальное значение удельного сопротивления равно '+FloatToStr(maxPizm)));
      ComboBox3.ItemIndex:=-1;
    end;
  end;
end;
...
begin
  ...
  if RadioButton1.Checked then Lp:=(n-1)*(ComboBox4.ItemIndex+1)*L
    else Lp:=n*(ComboBox4.ItemIndex+1)*L;
