...
void WhileTestSolve(void)
{
   ...
   while(true) {
      ...
      if (bl==0) { //ненайдено
         ...
         if ( !msg || (msg & EVN_STOP_SRV)>0 ) break; //завершение работы сервера
         continue;
      }
...
/*
      ...
      GetSystemTime(&sys_tm);
      sprintf(ts,"%02d:%02d:%02d.%03d",sys_tm.wHour,sys_tm.wMinute,sys_tm.wSecond,sys_tm.wMilliseconds);
      logf << ts << " [СЕРВЕР "<<DB.get_id() <<"] Завершена проверка решения " 
           << elm.id_stat << endl;
      ...
      GetSystemTime(&sys_tm);
      sprintf(ts,"%02d:%02d:%02d.%03d",sys_tm.wHour,sys_tm.wMinute,sys_tm.wSecond,sys_tm.wMilliseconds);
      logf << ts << " [СЕРВЕР "<<DB.get_id() <<"] Сохранен результат проверки решения " 
           << elm.id_stat << endl;
