...
int TDataBase::SqlExecute(char *query, isc_tr_handle trans)
{
...
int TDataBase::SelectDataSubmit(TSubmited *e, isc_tr_handle trans)
{
   ...
   if (rs==0) {
      ...
      sprintf(selstr,
         "select time_lim, mem_lim, min_uniq_proc from problems where id_prb=%d order by id_prb",
         e->id_prb);
...
BOOL TDataBase::TestingBestSolve(TSubmited *e, int *old_author, int *id_slv, isc_tr_handle trans)
{
   ...
   if (rs==100L) {
      ...
      *old_author=tmp_field;
      ...
      sprintf(s3,"%f",(float)e->time/1000.0);
...
int TDataBase::TestPlagiat(int id_stat, char *obj_file, float min_uniq_proc,
                           char *otch, int *again_plagiat, isc_tr_handle trans)
{
   char query[400],s[100],*bf0,*bf1,*bf_cur;
   ISC_QUAD blob_id;
   isc_blob_handle blob_handle=NULL;
   XSQLDA ISC_FAR *sqlda;
   ...
   //char s1[]="xxabcdexxxxpoiuyqwerxxx",s2[]="zabcdepoiuyqwerzzzzzzzzzzzzzzzzzzz";

   //sz_r = find_common_substr(s1,strlen(s1),s2,strlen(s2),5);
   if (min_uniq_proc<0.001) {
      return 1;
   }
   ...
   while (isc_dsql_fetch(status, &stmt, SQL_DIALECT_V6, sqlda)==0) {
      ...
      //for (int i=5;i<200;i+=5) {
         vl_uniq = find_common_substr(bf0,len0,bf1,len1,150);
      //   logf << i << ";   " << vl_uniq << "\n";
...
void TDataBase::print_error(char *s)
{
   ...
   logf << ts <<" [СЕРВЕР"<< id_srv <<"] ERROR DATABASE!!! errorcode="<< errcode << "\n";
   logf << s << "\n";
