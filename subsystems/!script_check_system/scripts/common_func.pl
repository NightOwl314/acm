...
#добавляет информацию о поддерживаемых компиляторах
sub insert_compilers
{
   ...
   $query="select id_cmp, name from compil order by id_cmp";
   $sth=$db->prepare($query);
   $sth->execute();
...
sub insert_status_rows
{
  ...
  my $before,$after,$sect_srow,$row_count,$s;
  my $add_index="",$add_prm_q="",$op='<';
  my $query,$sth,%prb_hash=(),%publ_hash=(),$iview,$single_author=0;
  ...
  #обычный статус
  if ($id_user>0 && $id_publ==0 && $id_rsl==0) {
...
SQL
     ...
     $query="select id_prb, f_help_prb from access_user_prb($id_user,'.".join('.',keys %prb_hash).".')";
...
#my debug
#     $s=~s/\$show_report\$\{(.*?)\|(.*?)\}/($publ_hash{$row[3]}>0 && $row[12]>0 && $prb_hash{$row[5]}>0 )?$1:$2/eig;
     $s=~s/\$show_report\$\{(.*?)\|(.*?)\}/($row[12]>0)?$1:$2/eig;
