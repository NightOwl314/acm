...
sub insert_ranklist
{
  my($text,$rank,$grp,$id_lng,$id_user) = @_;
...
sub access_source
{
   my($id_stat,$id_publ,$who_v_ptr,$id_prb_ptr)=@_;
...
sub insert_src_atr
{
  ...
  $query = "select s.id_publ, a.name, s.id_prb, p.name, c.name "
...
sub insert_problem_stat
{
  ...
  #извлекаем статистику результатов
  $query = "select r.name,s.cnt from statistica s, results_lng r "
  ...
  while (@row = $sth->fetchrow_array) {
     ...
     ."<td align=center><a href=/cgi-bin/statistica.pl?id_slv=$row[0]>$best_str</a></td>"
     ."</tr>\n";
  }
...
#       }
  ...
  if ($id_publ) {
    ...
    insert_status_rows(\$string_template,$id_lng,$id_stat,$id_prb);
  } elsif ($search_list ne "") {
   ...
   $string_template =~ s/\$add_param/&id_stat=$id_stat/ig;
    ...
    insert_ranklist(\$string_template,$rank,$p_grp,$id_lng,$id_user);
  }
