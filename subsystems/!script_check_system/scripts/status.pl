...
sub insert_report
{
   ...
   my $query,$sth,$id_rpt,$id_text,$title,$msg,$i,$prb,$rsl,$dsc_err='',$dsc_test='';
   my %pl_hash=(),$s,$sth1,$uname,$cname,$id_publ,$cur_src,$other_src,$test_nm;
  

   $$text=~s/\$stat_id/$id_stat/ig;
   
   $query="select s.id_stat,s.id_prb,s.id_rsl,s.id_publ,s.test_no from status s where s.id_stat=$id_stat";
   $sth = $db->prepare($query);
   $sth->execute;
   ($i,$prb,$rsl,$id_publ,$test_nm)=$sth->fetchrow_array;
   $sth->finish;
...
SQL
   ...
   if (1) {
      ...
      $query = <<SQL;
          select r.id_rpt,r.comment, rl.name, sr.text
          ...
          where sr.id_stat=$id_stat
SQL

      $sth = $db->prepare($query);
      $sth->execute;
      while (($id_rpt,$id_text,$title,$msg)=$sth->fetchrow_array) {
