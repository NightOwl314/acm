...
sub get_status_rows
{
  ...
  $query0="select status.id_stat, cast(status.dt_tm as date),cast(status.dt_tm as time), "
      ."status.id_publ, authors.name, status.id_prb, "
      ."results_lng.name,status.test_no, status.id_rsl "
      ."from status s "
      ."JOIN authors a ON a.id_publ = s.id_publ "
      ."JOIN results_lng r ON (r.id_rsl = s.id_rsl) and (r.id_lng=(select def_lng from const)) "
      ."where status.id_stat<$id_stat \$filter "
      ."PLAN JOIN (S INDEX (STATUS_IDX1), "
      ."A INDEX (RDB\$PRIMARY107), "
      ."R INDEX (RDB\$PRIMARY113)) "
      ."order by status.id_stat desc";
...
sub get_servers_status
{
   ...
   $query='select id_srv,upper(host_name),pid,processor_num,testing,start_time,last_test_time,testing_time
           from test_servers where id_srv in ('.join(',',0,@servers).') order by host_name';
