...
main_cik:
  ...
  if ($p_mode eq 'new_post' || $p_mode eq 'edit_post') {
     ...
     if ($p_mode eq 'new_post' || length($author_pwd)>0 || length($author_pwd2)>0) {
        ...
        if (!$err_code && length($author_pwd)<1) {
           $err_code=2;
        }
     }
     ...
     if (!$err_code) {
        ...
        if ($p_mode eq 'new_post') {
           ...
           #проверим кодовое слово
           $codeword = $incgi->param("codeword");
           if (length($codeword) > 0) {
             $query = "insert into groups_authors(id_grp, id_publ) select id_grp, $new_id_publ from groups where codeword='$codeword'";
             $sth=$db->prepare($query);
             $sth->execute();
           }
        ...
        if ($p_mode eq 'edit_post') {
           ...
           $codeword = $incgi->param("codeword");
           if (length($codeword) > 0) {
             $query = "insert into groups_authors(id_grp, id_publ) select id_grp, $p_id_publ from groups where codeword='$codeword'";
             $sth=$db->prepare($query);
             $sth->execute();
           }
