...
sub connect_db
{
 ...
 $db = DBI->connect("$dsn", "$user_db", "$password_db",
    {LongReadLen=>1048576, AutoCommit => 0, RaiseError => 0, PrintError => 0} );
...
sub current_page
{
  ...
  $$text =~ s/<!--start_assign_problems-->(.*?)<!--finish_assign_problems-->//sig;
...
sub login_info
{
  ...
  $$text =~ s/<!--start_assign_problems-->(.*?)<!--finish_assign_problems-->/is_can_assign_problems($user)>0?$1:''/esig;
...
sub insert_all_compilers
{
   ...
   $query=" select compil.id_cmp, compil.name, compil.id_serv, servers.name "
         ...
         ." order by compil.id_serv, compil.sort_order ";
   $sth=$db->prepare($query);
   $sth->execute();
...
sub LDAP_authenticate
{
  ...
  #only some prefixes
  #DEBUG
  if (!($user =~ /\d\d\d\d_/i)) {return "false";}
...
#  меняем пароль пользователя, так чтобы он соответствовал паролю в AD
    $query = "update authors set pwd='$password' where login='$user'";
    my $sth = $db->prepare($query);
    $sth->execute;
  }
...
SQL
  ...
  while ((@row = $sth->fetchrow_array) && $cnt<$row_count) {
     ...
     my $points_value = $row[19];
     if ($points_value == 0) {
       $points_value = "&nbsp;";
     }
     $s=~s/\$points/$points_value/ig;
     
     my $comment_value = $row[20];
     $s=~s/\$comment/$comment_value/ig;
...
sub save_submit
{
   my ($err_list,$id_user,$id_prb,$id_cmp,$source_text,$source_file_nm,$p_comment)=@_;
...
#      $p_comment=$db->quote($p_comment);
      $query="insert into status(id_publ,id_prb,id_cmp,id_rsl,comment) values($id_user,$id_prb,$id_cmp,100,?)";
      $sth=$db->prepare($query);
      $sth->execute($p_comment);
#      to_log($p_comment);
...
#------------------------------------------------------------------------
sub is_can_assign_problems
{
  my ($iuser) = @_;
  my $query,$sth,$rez=0;

  $query=<<SQL;
      select max(g.f_assign_prb)
