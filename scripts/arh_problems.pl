...
SQL
	...
	#если прогноз невозможен
	if($id_user == 0){
	    my $help = "<div id='help' style='background-color: #fef0b3; border: solid 1px #cc0075; width:200px; color:#cc0075; margin-bottom: 5px; text-align: center; padding:5px'>Прогноз невозможен, зайдите с паролем или зарегистрируйтесь
		            <br>
					<input type='button' value='Закрыть' onclick='CloseHelp()'></input> 
					</div><div></div>";
	   ...
	   my $k = 0;
		...
		my $help = "<div id='help' style='background-color: #fef0b3; border: solid 1px #cc0075; width:200px; color:#cc0075; margin-bottom: 5px; text-align: center; padding:5px'>Вы решили слишком мало задач, решите еще $add_task задач для получения прогноза
		            <br>
					<input type='button' value='Закрыть' onclick='CloseHelp()'></input> 
					</div><div></div>";
		$$text =~ s/\$helper/$help/ig;
		...
		foreach(@classes){
			if ($classes[$j] == 3){
				$$text =~ s/\$class$task[$j]task/Легкая/ig;
			}elsif($classes[$j] == 2){
				$$text =~ s/\$class$task[$j]task/Средней сложности/ig;
			}elsif($classes[$j] == 1){
				$$text =~ s/\$class$task[$j]task/Трудная/ig;
			}elsif($classes[$j] == 0){
				$$text =~ s/\$class$task[$j]task/Уже решена/ig;
			}else{
				$$text =~ s/\$class$task[$j]task/Прогноз невозможен/ig;
			}
			$j++;
		}
		$$text =~ s/\$helper//ig;
...
SQL
  ...
  while ( my @row = $sth->fetchrow_array) {
    ...
    if (($row[5]==$user || $row[4]>0) && !$classify) {    
        ...
        ."</tr>\n";
		my $prognoz = "<th width='60'>Прогноз</th>";
