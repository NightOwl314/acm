...
#   to_log("request from user");
   ...
   %templ_hash = (new         => "add_problem_$id_lng.html",
                  ...
                  edit_cond   => "edit_cond_prb_$id_lng.html",
                  edit_cond_post=> "edit_cond_prb_$id_lng.html",
   ...
   if (exists_rec($p_id_prb,'problems')) {
      if (!($p_mode eq "edit"     || 
            ...
            $p_mode eq "edit_test" || 
            $p_mode eq "new_test" || 
            $p_mode eq "edit_test_post" ||
            $p_mode eq "edit_cond" ||
            $p_mode eq "edit_cond_post") ) {
         $p_mode="edit";
      }
   } else {
      if (($p_mode eq "edit" || 
...
#            to_log($new_problem_url);
      ...
      if ($p_mode eq "new") {
         ...
         $incgi->param(-name=>'mem_limit',-value=>'65536');
         $incgi->param(-name=>'min_uniq_proc',-value=>'30');
         $incgi->param(-name=>'hardlevel',-value=>'10');
      }
...
#   my $p_id_prb=$incgi->param("id_prb")+0;
      insert_compilers(\$string_template);
      insert_all_compilers(\$string_template);
   ...
   } elsif ($p_mode eq "edit_cond" || $p_mode eq "edit_cond_post") {

      if ($p_mode eq "edit_cond_post") {
         save_cond($p_id_prb);
         $new_url='/cgi-bin/arh_problems.pl';
         $new_url =~ s/\?.*$//;
         $new_url.="?id_prb=$p_id_prb";
         print header(-status=>"301 Moved Permanently",
                      -Location=>$new_url);
         next main_cik;
      }
      insert_cond_p(\$string_template,$p_id_prb,$id_lng);
...
sub insert_subjects_p
{
   ...
   if ($id_problem) {
      $query=<<SQL;
      select tree.id_tm,tree.id_level,max(tree.is_enbl),tl.name,tp.id_prb,tp.order_pos 
         ...
         group by tree.n_ord,tree.id_tm,tree.id_level,tl.name,tp.id_prb,tp.order_pos
         order by tree.n_ord
SQL
   } else {
      $query=<<SQL;
      select tree.id_tm,tree.id_level,max(tree.is_enbl),tl.name,null 
...
SQL
   ...
   while (my @row=$sth->fetchrow_array) {
      ...
      $s =~ s/\$order_pos_tm/$row[5]/ig;
      $s =~ s/\$offset_tema/$offset/ig;
      
      if ($id_problem) {
         $s =~ s/\$tema_selected\$\{(.*?)\|(.*?)\}/$row[4]?$1:$2/eig;
      }
...
sub save_problem
{
   ...
   my $fl_lst,$list_tests_fn,$std_checker,$checker_file_exe,$checker_file_scr,$chk_compil,$id_compil,$list_points_fn;
   my @del_files=(),$time_limit,$mem_limit,$new_id_prb,@row,$valid_solve,$user_sbm;
   my $s,%enbl_tests=(),@set=(),$min_uniq_proc,@exts=('in','out','img'),%tm_order_pos=();
   my %hardlevel=10;
...
SQL
      ...
      while(($i)=$sth->fetchrow_array) {
         push(@subjs,($i));
         $tm_order_pos{$i}=$incgi->param("tm_order_pos_$i");
      }
      $sth->finish();
      $s=join(',',0,@subjs);
...
#      $stat_err=5 if ($i);
   ...
   if (!$stat_err && $tests_file) {
      ...
      if ($i) {
         ...
         $list_points_fn="$tests_dir\\".$ProblemPaths{'ListPoints'};
   ...
   #проверка уровня сложности задачи
   if (!$stat_err) {
      $hardlevel=int($incgi->param('hardlevel')+0);
      $stat_err=99 unless ($hardlevel>=0 && $hardlevel<=100);
   }
   ...
   if (!$stat_err && $$ptr_id_prb) {
      ...
      $query="update problems set time_lim=$time_limit, mem_lim=$mem_limit, "
            ."min_uniq_proc=$min_uniq_proc, hardlevel=$hardlevel where id_prb = $$ptr_id_prb";
      $sth=$db->prepare($query);
      $sth->execute();
      ...
      $query="insert into tm_prb(id_tm,id_prb,order_pos) values(?,$$ptr_id_prb,?)";
      $sth=$db->prepare($query);
      foreach $k (@subjs) {
         $sth->execute($k,$tm_order_pos{$k});
      }
      ...
      if (!-d "$DirPrbCond\\$$ptr_id_prb") {
         ...
         mkdir("$DirPrbCond\\$$ptr_id_prb\\en"); #debug
         mkdir("$DirPrbCond\\$$ptr_id_prb\\ru"); #debug
      }
      ...
      if ($tests_file) {
         ...
         rename($list_points_fn,"$DirProblems\\$$ptr_id_prb\\".$ProblemPaths{'ListPoints'});
         ...
         re_pack("$DirProblems\\$$ptr_id_prb\\".$ProblemPaths{'ArchivTestsName'},
                 "$DirProblems\\$$ptr_id_prb\\".$ProblemPaths{'ListTests'},
                 "$DirProblems\\$$ptr_id_prb\\".$ProblemPaths{'ListPoints'},
                 @exts);
      }

      #сохраняем включенные тесты, если не указан архив тестов
      if (!$tests_file) {
         $fln="$DirProblems\\$$ptr_id_prb\\".$ProblemPaths{'ListTests'};
         $s='';
         $fl_lst=new IO::File;
   ...
   if (!$stat_err && !($$ptr_id_prb)) {
      ...
      $query="insert into problems(time_lim,mem_lim,id_creator,min_uniq_proc,hardlevel,id_serv) "
            ."values($time_limit,$mem_limit,$id_user,$min_uniq_proc,$hardlevel,(select id_serv from compil where id_cmp=$id_compil))";
      $sth=$db->prepare($query);
      $sth->execute();
      ...
      #сохраним темы
      $query="insert into tm_prb(id_tm,id_prb,order_pos) values(?,$new_id_prb,?)";
      $sth=$db->prepare($query);
      foreach $k (@subjs) {
         $sth->execute($k,$tm_order_pos{$k});
      }
      ...
      rename($list_points_fn,"$DirProblems\\$new_id_prb\\".$ProblemPaths{'ListPoints'});

      #запакуем все тесты в архив
      grep($_="$DirProblems\\$new_id_prb\\".$ProblemPaths{'Tests'}."\\*.$_",@exts);
      re_pack("$DirProblems\\$new_id_prb\\".$ProblemPaths{'ArchivTestsName'},
              "$DirProblems\\$new_id_prb\\".$ProblemPaths{'ListTests'},
              "$DirProblems\\$new_id_prb\\".$ProblemPaths{'ListPoints'},
              @exts);
...
sub insert_limits_p
{
   ...
   $query="select time_lim,mem_lim,min_uniq_proc,hardlevel from problems where id_prb=$id_problem";
   $sth=$db->prepare($query);
   $sth->execute();
   ($time_limit,$mem_limit,$min_uniq_proc,$hardlevel) = $sth->fetchrow_array;
   $sth->finish();
