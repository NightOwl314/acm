...
while ( next_request() ) {
	...
	if ($id_user) {
		...
		print_err("Not allowed for guest");
	}
}
...
sub insert_themes_list {
	...
	my $add_text = "";
		$query=<<SQL;
  		...
  		where assigned_problems.id_user = $id_user
...
SQL
	...
	$add_text .= "<table class = \"themesTable\">
		 <col id =\"c_one\">
   		 <col id = \"c_two\" >";
		$add_text .= "<tr><td align =\"center\" onclick = \"getStProblemsForThemes(this, 0);\"><p>+</p></td>";
		$add_text .= "<td><a href =\"arh_problems.pl\">Все назначенные задачи</a></td></tr>";
		$add_text .= "</table>";
	...
	while (@row = $sth->fetchrow_array) {
		...
		<col id =\"c_one\">
   		 <col id = \"c_two\" >";
		$add_text .= "<tr><td align =\"center\" onclick = \"getStProblemsForThemes(this, $row[0]);\"><p>+</p></td>";
