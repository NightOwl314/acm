...
SQL
  ...
  while (($cnt<$rec_cnt) && (@row = $sth->fetchrow_array)) {
     ...
     ."<td align=center>$row[5]</td>"
      ...
      if($grp > 0) {
      #out information about solved problems of author
      $new_text .= "<tr><td colspan = 6>";
      #получаем информацию о задачах, решенных данным автором в данной и нижележащих темах
      $query=<<SQL;
      select status.id_prb, max(id_stat) as id_stat, max(problems_lng.name) as name, min(100-uniq_proc) as plag_percent from status inner join tm_prb
      on status.id_prb=tm_prb.id_prb
      inner join get_tree_subj($topic, 1) gts
      on tm_prb.id_tm=gts.id_tm
      inner join problems_lng
      on problems_lng.id_prb = status.id_prb
      where status.id_rsl=0 and status.id_publ=$row[1] and problems_lng.id_lng='$id_lng'
      group by status.id_prb
      order by status.id_prb
SQL
#      to_log($query);
      $cur = $db->prepare($query);
      $cur->execute;
      while(@rec = $cur->fetchrow_array) {
        $name_prb = trim($rec[2]);
        $new_text .= "<a href=status.pl?id_prb=$rec[0]&id_publ=$row[1] title='$name_prb'>$rec[0]";
        if ($rec[3] >= 70) {
         $plag_percent = int($rec[3] + 0.5);
         $new_text .= "<sup><font size=-2 color=yellow>$plag_percent</font></sup>";
        }
        $new_text .= "</a> "
      }
