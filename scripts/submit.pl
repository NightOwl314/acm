...
while(next_request()) {
   ...
   $p_comment=$incgi->param("comment");
   $p_comment=encode_entities($p_comment, '<>"');
#   to_log($p_comment);
   ...
   if ($p_mode eq 'send') {
      ...
      $err_sect=$1.$3;
      $err_def=$2;
      $err_text=save_submit($err_def,$id_user,$p_id_prb,$p_id_compiler,$p_source,"sourcefile",$p_comment);
      if ($err_text) {
   ...
   $string_template =~ s/\$comment_text/$p_comment/ig;
